...
const juniorWordbooks = [
  ...
  {
    "id": "junior_7th_ren_jiao_new",
    "title": "人教版英语七年级上册（新版）",
    "description": "按新版教材 Vocabulary in Each Unit 顺序整理，共332词",
    "category": "junior",
    "grade": "7th",
    "region": "全国",
    "version": "人教版",
  ...
  {
    "id": "junior_7th_ren_jiao_second_new",
    "title": "人教版英语七年级下册（新版）",
    "description": "按新版教材 Vocabulary in Each Unit 顺序整理，共462词",
    "category": "junior",
    "grade": "7th",
    "region": "全国",
    "version": "人教版",
  ...
  {
    "id": "junior_8th_ren_jiao_new",
    "title": "人教版英语八年级上册（新版）",
    "description": "按新版教材 Vocabulary in Each Unit 顺序整理，共551词",
    "category": "junior",
    "grade": "8th",
    "region": "全国",
    "version": "人教版",
  ...
  {
    "id": "junior_9th_ren_jiao_new",
    "title": "人教版英语九年级上册（新版）",
    "description": "按新版教材 Vocabulary in Each Unit 顺序整理，共485词",
    "category": "junior",
    "grade": "9th",
    "region": "全国",
    "version": "人教版",
  ...
  {
    "id": "junior_9th_ren_jiao_full_new",
    "title": "人教版英语九年级全一册（新版）",
    "description": "按新版教材 Words and Expressions in Each Unit 顺序整理，共604词",
    "category": "junior",
    "grade": "9th",
    "region": "全国",
    "version": "人教版",
...
const generateWordsForBook = function (bookCategory, bookId, startIndex, count) {
  ...
  // 直接根据词书类型加载对应的JS文件，避免JSON加载的兼容性问题
    try {
      ...
      } else if (bookId === 'junior_7th_ren_jiao_new') {
        const cloudWords = cloudWordbookLoader.getWordsSync(bookId);
        if (cloudWords && cloudWords.length > 0) {
          words = cloudWords;
          console.log('成功从云端缓存加载人教版英语七年级上册（新版），数量：', words.length);
        } else {
          console.log('人教版英语七年级上册（新版）云端缓存未命中，使用词书默认单词，数量：', book.words ? book.words.length : 0);
          words = book.words || [];
        }
      } else if (bookId === 'junior_7th_ren_jiao_second_new') {
        const cloudWords = cloudWordbookLoader.getWordsSync(bookId);
        if (cloudWords && cloudWords.length > 0) {
          words = cloudWords;
          console.log('成功从云端缓存加载人教版英语七年级下册（新版），数量：', words.length);
        } else {
          console.log('人教版英语七年级下册（新版）云端缓存未命中，使用词书默认单词，数量：', book.words ? book.words.length : 0);
          words = book.words || [];
        }
      } else if (bookId === 'junior_8th_ren_jiao_new') {
        const cloudWords = cloudWordbookLoader.getWordsSync(bookId);
        if (cloudWords && cloudWords.length > 0) {
          words = cloudWords;
      ...
      } else if (bookId === 'junior_9th_ren_jiao_new') {
        const cloudWords = cloudWordbookLoader.getWordsSync(bookId);
        if (cloudWords && cloudWords.length > 0) {
          words = cloudWords;
          console.log('成功从云端缓存加载人教版英语九年级上册（新版），数量：', words.length);
        } else {
          console.log('人教版英语九年级上册（新版）云端缓存未命中，使用词书默认单词，数量：', book.words ? book.words.length : 0);
          words = book.words || [];
        }
      } else if (bookId === 'junior_9th_ren_jiao_full_new') {
        const cloudWords = cloudWordbookLoader.getWordsSync(bookId);
        if (cloudWords && cloudWords.length > 0) {
          words = cloudWords;
          console.log('成功从云端缓存加载人教版英语九年级全一册（新版），数量：', words.length);
        } else {
          console.log('人教版英语九年级全一册（新版）云端缓存未命中，使用词书默认单词，数量：', book.words ? book.words.length : 0);
          words = book.words || [];
        }
