...
- 统一统计引擎将“当前 difficult”与“尚未完成五轮的 preview_not_mastered/旧版抗遗忘种子”合并为未掌握集合；第五轮完成后不再计数。首页和详细统计继续共用该引擎，避免页面各算一套。
- 复习保存时把 `updatedAt` 设置为本次复习时间；`syncMasteryAtom` 在事务内再以获胜记录的语义活动时间兜底修正 `updatedAt`，兼容未升级客户端。
- 云端合并仍优先比较 `reviewCount` 和真实活动时间，不使用单纯的服务器接收时间制造虚假的新版本。
- 预习点击先持久化当前词的排除状态，再构造并上传同一快照；普通空数组继续采用兼容合并，只有携带 `reset=true` 的学习闭环才有权清空云端 `order/excluded`，且该标志进入失败重试队列。
