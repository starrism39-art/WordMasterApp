...
function membershipModel(row, now, config, pending = false) {
  ...
  const reminders = reminderModel(row, access, now);
  const {remainingDays} = reminders;
  ...
  const entrySubtitle = displayState === 'free' ? '免费版' : displayState === 'active' ? '有效期至 ' + expiry :
    displayState === 'expiring' ? (remainingDays === 0 ? '今天到期' : remainingDays + ' 天后到期') : displayState === 'transition' ? '缓冲期剩余 ' + transitionRemainingDays + ' 天' : displayState === 'long_term' ? '长期有效' : '已到期';
  ...
  return { ...publicConfig(config), ...reminders, displayState, statusLabel:labels[displayState], title:titles[displayState], entrySubtitle,
    attention: !!reminders.reminder || needsRetentionSelection, summary:summaries[displayState], notice:notices[displayState],
